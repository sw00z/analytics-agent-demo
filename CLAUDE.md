...
Read [README.md](./README.md) for the high-level pitch and technical writeup, and [DESIGN.md](./DESIGN.md) for the editorial voice and visual register.
...
## Design system (DESIGN.md / DESIGN.json)

Tokens, component compositions, and named rules live in [DESIGN.md](./DESIGN.md) (prose, six sections) and [DESIGN.json](./DESIGN.json) (structured form with tonal ramps, motion easings, shadow vocabulary, and per-component recipes). [`app/globals.css`](./app/globals.css) is the implementation surface — drift between it and DESIGN.md is the most common silent bug class on the visual side.

**Read DESIGN.md before any frontend, styling, or microcopy change.** Compose components from the named tokens in its YAML frontmatter; never reach for raw OKLCH, raw pixel values, or Tailwind colours outside the palette. Look up tonal ramps and alpha variants in DESIGN.json under `extensions.colorMeta.<token>`.

The §2 / §3 / §4 rules are contracts; the §6 anti-pattern list is enforceable. If a user request would land in §6, propose a system-coherent alternative rather than implementing it.
